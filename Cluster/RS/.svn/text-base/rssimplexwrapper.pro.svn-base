Pro rssimplexwrapper, obj, start=start, scale=scale, magrange=magrange, colorrange=colorrange
...
;  if n_elements(start) eq 0 then start = [0.946, -0.028, 0.028, 5., 22., -1., -7., 0.7, -3., -0.007]
  ...
  result = downhillsimplex( 'LogLikelihood', start, scale, object = rslike, $
                            tol=0.001, miniter=100 )
  for i=0, 10 do begin
     result = downhillsimplex( 'LogLikelihood', result, scale, object = rslike, $
                               tol=0.001, miniter=100, fval=fval )
