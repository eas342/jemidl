Function AddSpectra, $
   ...
   sexcat_ptr ;; sextractor catalog
  ;; assume there is only 1 *spec.cat in the directory

  thisexcept = !except
  !except=0

  if strmid( dir, strlen(dir)-1, 1 ) ne '/' then dir += '/'
  SpecCatFile = file_search( dir+'*spec.cat' )
  openr, lun, SpecCatFile, /get_lun
  f=''
  readf, lun, f, format='(A)' ;;get the format
  format=f
  ...
  ;; now calculate shift
  for i=0, ngals-1 do begin
     dist2 = ((*sexcat_ptr).alphawin_j2000 - ra[i])^2*cos( dec[i]*!pi/180. )^2 $
             + ((*sexcat_ptr).deltawin_j2000 - dec[i])^2
     md2 = min( dist2, m )*3600.^2
     shiftra[i]  = ( ra[i]  - (*sexcat_ptr)[m].alphawin_j2000 )*3600.
     shiftdec[i] = ( dec[i] - (*sexcat_ptr)[m].deltawin_j2000 )*3600.
  endfor
  ...
  case format of
     'DEIMOS' : begin
        for i=0, ngals-1 do begin
           ...
           dist2 = ((*sexcat_ptr).alphawin_j2000 - (ra[i] - medshiftra))^2*cos( dec[i]*!pi/180. )^2 $
                   + ((*sexcat_ptr).deltawin_j2000 - (dec[i] - medshiftdec))^2
           ...
           if md2 lt 1.0^2 then begin ;; found in the ACS image
              clusterid = (*sexcat_ptr)[m].clusterid
              galid = (*sexcat_ptr)[m].galid
           endif else begin ;; not identified in the ACS image
              clusterid = (*sexcat_ptr)[0].clusterid
              galid = -1
           endelse
