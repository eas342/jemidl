...
IDL_VPTR clean(int argc, IDL_VPTR *argv)
{
  ...
  //this will be the original image and then the residual after each iteration...
  for(i=0;i<NX*NY;i++){
  ...
  for (iter=0; iter<niter; iter++) {
    ...
    //SUBTRACT SCALED PSF AND ADD TO GUASSIMAGE
    for(j=0; j<NYp; j++) {
      for(i=0; i<NXp; i++) {
	iii=ipk-ipsf0+i;
	jjj=jpk-jpsf0+j;
    ...
    if (iter%defPRINTITER==0) {
      ...
      if (vmax<sqrt(var)*defSIGCUT) break;
    }
  }
