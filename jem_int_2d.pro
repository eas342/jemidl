Function JEM_int_2d_getval2, fcn, obj, x, y
  if ~obj_valid(obj) then begin
     return, call_function( fcn, x, y )
  endif else begin
     return, call_method( fcn, obj, x, y )
  endelse
end

Function JEM_int_2d_getval1, fcn, obj, x
  if ~obj_valid(obj) then begin
     return, call_function( fcn, x )
  endif else begin
     return, call_method( fcn, obj, x )
  endelse
end
; $Id: int_2d.pro,v 1.17 2005/02/01 20:24:26 scottm Exp $
;
...
FUNCTION JEM_Int_2D, Fxy, AB_Limits, PQ_Limits, Pts, Double = Double, Order = Order, $
                 Object = Object
...
if Pts eq 6 then begin
  ...
  if KEYWORD_SET(Order) eq 0 then begin
    ...
    for i = 0, Pts-1 do begin
      ...
      fLimit = JEM_int_2d_getval1(PQ_Limits, object, X)
;      fLimit = CALL_FUNCTION(PQ_Limits, X)
      kF = (fLimit[1] - fLimit[0]) / 2.0
      kS = (fLimit[1] + fLimit[0]) / 2.0
      jX = TOTAL(Wi * JEM_int_2d_getval2(Fxy, object, X, kF*Ri+kS), Double = Double)
;      jX = TOTAL(Wi * CALL_FUNCTION(Fxy, X, kF*Ri+kS), Double = Double)
    ...
    for i = 0, Pts-1 do begin
      ...
      fLimit = JEM_int_2d_getval1(PQ_Limits, object, Y)
;      fLimit = CALL_FUNCTION(PQ_Limits, Y)
      kF = (fLimit[1] - fLimit[0]) / 2.0
      kS = (fLimit[1] + fLimit[0]) / 2.0
      jY = TOTAL(Wi * JEM_int_2d_getval2(Fxy, object, kF*Ri+kS, Y), Double = Double)
