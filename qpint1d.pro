...
pro qpint1d_qagse, f, a0, b0, result, abserr, private, functargs=fa, $
                   ...
                   elist=elist, iord=iord, object=object
...
;           FIRST APPROXIMATION TO THE INTEGRAL
  ...
  for i = 0L,nint-1 do begin
      ...
      qpint1d_qkeval, f,a1,b1,area1,error1,defabs,resa, private, $ ;;here
        neval=neval, functargs=fa, inflow=bound, dirsign=dirsign, $
        isdouble=isdouble, npoints=npoints, object=object
...
;           ERROR ESTIMATE.
      ...
      b2 = blist(maxerr-1)
      erlast = errmax
      qpint1d_qkeval, f,a1,b1,area1,error1,resa,defab1, private, $ ;;HERE
        neval=neval, functargs=fa, inflow=bound, dirsign=dirsign, $
        isdouble=isdouble, npoints=npoints, object=object
      qpint1d_qkeval, f,a2,b2,area2,error2,resa,defab2, private, $ ;;HERE
        neval=neval, functargs=fa, inflow=bound, dirsign=dirsign, $
        isdouble=isdouble, npoints=npoints, object=object
...
pro qpint1d_qkeval, f, a, b, result, abserr, resabs, resasc, priv, $   ;;HERE
                  functargs=fa, neval=neval, reset=reset, isdouble=isdouble, $
                  inflow=bound, dirsign=dirsign, npoints=npts0, object=object
...
;  stop
  if n_elements(priv) GT 0 then begin
     if n_elements(fa) GT 0 then begin
        if ~obj_valid(object) then fv = call_function(f, x, priv, _EXTRA=fa) $ ;;HERE
        else fv = call_method(f, object, x, priv, _Extra=fa)
     endif else begin
        if ~obj_valid(object) then fv = call_function(f, x, priv) $
        else fv = call_method(f, object, x, priv)
     endelse
  endif else begin
      if n_elements(fa) GT 0 then begin
         if ~obj_valid(object) then fv = call_function(f, x, _EXTRA=fa) $
         else fv = call_method(f, object, x, _EXTRA=fa)
      endif else begin
         if ~obj_valid(object) then fv = call_function(f, x) $
         else fv = call_method(f, object, x)
      endelse
  endelse
...
function qpint1d, f0, a0, b0, private, npoints=npoints, expression=expr, $
                  ...
                  alist=alist, blist=blist, rlist=rlist, $
                  elist=elist, iord=iord, nocatch=nocatch, object=object
...
;  prof_start = systime(1)
  ...
  if a1 NE b1 then begin
      qpint1d_qagse, f, a1, b1, result, abserr, private, functargs=fa, $ ;;HERE
  ...
  if a2 NE b2 then begin
      qpint1d_qagse, f, a2, b2, result2, abserr2, private, functargs=fa, $ ;;HERE
        epsabs=epsabs/ef, epsrel=epsrel/ef, breakpoints=bpoints0, $
