...
;------------------------------------------------------------------------------------
Function jem_zcompute3, objwave, objflux, objivar, $
                        starflux=starflux, logwave0=logwave0, dlogwave=dlogwave, $
                        zmin=zmin, zmax=zmax, npoly=npoly, nfind=nfind, $
                        minsep=minsep, width=width, mindof=mindof, $
                        chi2arr=chi2arr, dofarr=dofarr, ztry=ztry, $
                        silent=silent, synflux=synflux, $
                        galtemplate=galtemplate, galwave=galwave
  ...
  if n_elements(objivar) eq 0 then objivar=fltarr( nobjpix, nobj )+1.0
  ...
  binflux = fltarr( nbin, nobj )
  binivar = fltarr( nbin, nobj )
  ...
  ;;ensure at least mindof pixels overlap
  ...
  else zmax=flotarr(nobjs)+max(ztry)
  nz=n_elements(ztry)
  ...
  if (npoly ne 0) then begin
     fixed_template = cheb_array( npoly-1, alog10(binwave) )
  ...
  if n_elements(galtemplate) ne 0 then begin
     ndim = size(galtemplate, /n_dimen)
     if ndim eq 1 then ngaltemplate=1 else ngaltemplate=(size(galtemplate, /dim))[1]
     if n_elements(fixed_template) ne 0 then begin
        for i=0, ngaltemplate-1 do begin
           fixed_template=[[fixed_template], $
                           [interpol(galtemplate[*,i], galwave, binwave)]]
           fixed_template2=[[fixed_template2], $
                           [interpol(galtemplate[*,i], galwave, objwave)]]
        endfor
     endif else begin
        fixed_template=interpol(galtemplate[*,0], galwave, binwave)
        fixed_template2=interpol(galtemplate[*,0], galwave, objwave)
        for i=1, ngaltemplate-1 do begin
           fixed_template=[[fixed_template], $
                           [interpol(galtemplate[*,i], galwave, binwave)]]
           fixed_template2=[[fixed_template2], $
                           [interpol(galtemplate[*,i], galwave, objwave)]]
        ...
        if ishift ge minind[iobj] and ishift le maxind[iobj] then begin
           ...
           if keyword_set(fixed_template) then $
              ...
              computechi2(binflux[idx,iobj],sqobjivar[idx,iobj], $
                          ([[template1[idx,*]],[fixed_template[idx,*]]]), $
                          dof=dof) $
           else $
              chi2arr[ishift,iobj] = $
              computechi2(binflux[idx,iobj],sqobjivar[idx,iobj], $
  ...
  ;; go back and fill in output arrays
  for iobj=0L, nobj-1 do begin
     zans1=create_zans(nstar+npoly+ngaltemplate,nfind)
