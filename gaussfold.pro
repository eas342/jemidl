...
FUNCTION gaussfold, lam, flux, fwhm, lammin=lammin, lammax=lammax, oversampfactor=oversampfactor
   ...
   IF n_elements(oversampfactor) eq 0 then oversampfactor=17.0

   dlambda   = fwhm / oversampfactor

   interlam  = lammin + dlambda * DINDGEN( LONG( (lammax-lammin)/dlambda+1 ))
   interflux = INTERPOL( flux, lam, interlam )

   fwhm_pix = fwhm / dlambda
   window   = FIX( oversampfactor * fwhm_pix )
