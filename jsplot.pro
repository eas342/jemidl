...
;   27-May-2004  add plugin keyword for coordinate plugins -
;   D. Finkbeiner
;   04-Mar-2010  Added /nan keyword to yrange minmax() - J. Meyers
;-
;------------------------------------------------------------------------------
...
pro splot_startup, xoffset=xoffset, yoffset=yoffset
   ...
   state = {                         $
    version: '$Name: v5_2_0 $'            , $ ; Version number of this release
...
pro splot_autoscale_y
   ...
   if (nplot GT 0) then begin
      ...
      if (igood[0] NE -1) then begin
         state.yrange[0] = min( (*plot_ptr[0]).y[igood], /nan )
         state.yrange[1] = max( (*plot_ptr[0]).y[igood], /nan )
      endif
      ...
      for i=1, nplot-1 do begin
         if ((*plot_ptr[i]).type EQ 'points') then begin
            ...
            if (igood[0] NE -1) then begin
               state.yrange[0] = $
                min( [state.yrange[0], (*plot_ptr[i]).y[igood]], /nan )
               state.yrange[1] = $
                max( [state.yrange[1], (*plot_ptr[i]).y[igood]], /nan )
            endif
         endif
      endfor
   endif
