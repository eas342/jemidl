...
;   10-Feb-2012  added chekfile option
;-
;------------------------------------------------------------------------------
...
;;;;;;Change 1: replace all negative wavelengths with a value of 1.0
  ...
  IF KEYWORD_SET(FLIP) THEN BEGIN
     ...
     insky    = reverse(insky)
  ENDIF
  hand_mask = fix(influx, type=1)*0B ;; make a bytarr the same size as influx

;; josh hack for hand masked CRs
  if n_elements(badpixfile) ne 0 then begin
     if (file_info(badpixfile)).exists then begin
        ...
        for i=0, n_elements(infiles)-1 do begin
           w=(where(infiles[i] eq badpixeldata.filename))[0]
           if w eq -1 then continue
;         inivar[*badpixeldata[i].badpixels, i] = 0.
;         influx[*badpixeldata[i].badpixels, i] = !values.f_nan
           hand_mask[*badpixeldata[w].badpixels, i] = 1
        endfor
     endif
  endif

  jm_long_combspec, influx, inivar, inloglam, insky = insky, innivar = innivar $
                    , newloglam = newloglam, newflux = newflux $
                    , newivar = newivar, newnivar = newnivar $
                    , newmask = newmask, newsky = newsky $
                    , iref = iref, SIGREJ = SIGREJ, CHECK = CHECK, CHEKFILE = CHEKFILE $
...
;      sxaddpar, head0, 'FLX_SHFT_WAV', flx_shft_wav
     ...
     sxaddpar, head0, 'NCOMBINE', n_elements(infiles)
     for i=0, n_elements(infiles)-1 do begin
        key='INFILE'+strn(i,f='(I02)')
        value=(reverse(strsplit(infiles[i], '/', /extract)))[0]
        sxaddpar, head0, key, value
     endfor
