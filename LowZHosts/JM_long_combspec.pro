...
PRO JM_LONG_COMBSPEC, influx, inivar, loglam $
                      ...
                      , iref = iref1, SIGREJ = SIGREJ1, CHECK = CHECK, CHEKFILE = CHEKFILE $
...
;; Weight all pixels and all exposures equally
  IF KEYWORD_SET(EQUAL_WEIGHT) THEN sn_weights[*] = 1.0d
...
;    ELSE sigrej_final = sigrej
     ...
     FOR ii = 0L, niter-1L DO BEGIN
        ;; josh hack, hand mask CRs
        IF N_ELEMENTS(hand_mask) ne 0 THEN BEGIN
           outmask or= hand_mask
        ENDIF
        ;; end josh hack
...
;;            outmask[*, j] = (outmask_temp EQ 0)
           ...
           IF KEYWORD_SET(CHECK) AND (ii EQ niter-1L) THEN BEGIN
              ...
              ELSE x_splot, 10.0D^newloglam, new_in $
                            ...
                            , YMNX = [ref_min, ref_max]
           ENDIF
           if n_elements(chekfile) ne 0 and (ii eq niter-1L) then begin
              if n_elements(hand_mask) eq 0 then begin
                 ind_bad = WHERE(arrmask[*, j] EQ 0 AND outmask[*, j] EQ 1 $
                                 , nbad)
              endif else begin
                 ind_bad = WHERE(arrmask[*, j] EQ 0 AND (outmask[*, j] EQ 1 $
              ...
              if j eq 0 then begin
                 checkout = { $
                            wave:10^newloglam, $
                            coadd:new_in, $
                            indiv:dblarr(nimgs, n_elements(arr_in)), $
                            mask:bytarr(nimgs, n_elements(arr_in)), $
                            var:dblarr(nimgs, n_elements(var_tot)) $
                            }
              endif
              checkout.indiv[j,*] = arr_in
              if nbad ne 0 then checkout.mask[j,*] = member(indgen(n_elements(arr_in)), ind_bad)
              checkout.var[j,*] = var_tot
              if j eq nimgs-1 then mwrfits, checkout, chekfile, /create
           endif
        ENDFOR
     ENDFOR
  ENDIF
...
;; Combine the spectra by taking a SNR weighted average.
  ...
  IF KEYWORD_SET(CHECK) THEN BEGIN
     loadct, 0
     color_vec = lonarr(10)
     color_vec[0] = fsc_color('white', 255, /check_connection)
     color_vec[1] = fsc_color('red', 240, /check_connection)
     color_vec[2] = fsc_color('green', 241, /check_connection)
     color_vec[3] = fsc_color('blue', 242, /check_connection)
     color_vec[4] = fsc_color('cyan', 243, /check_connection)
     color_vec[5] = fsc_color('magenta', 244, /check_connection)
     color_vec[6] = fsc_color('yellow', 245, /check_connection)
     color_vec[7] = fsc_color('orange', 246, /check_connection)
     color_vec[8] = fsc_color('purple', 247, /check_connection)
     color_vec[9] = fsc_color('tan', 248, /check_connection)
     ...
     plot, 10.0d^newloglam, imweights[*, 0] $
           ...
           , background = fsc_color('black', 0, /check_connection)
